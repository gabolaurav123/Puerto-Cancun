...
function renderPropertyPage(property, lang = "es", similar = []) {
  ...
  const mapSection = mapQuery
    ? `<section class="property-location-map"><div class="property-location-map-heading"><div><span class="seo-eyebrow">${english ? "Location" : "Ubicacion"}</span><h2>${english ? "Property location" : "Ubicacion de la propiedad"}</h2></div>${publicMapUrl ? `<a class="text-link" href="${escapeHtml(publicMapUrl)}" target="_blank" rel="noopener">${english ? "Open in Google Maps" : "Abrir en Google Maps"}</a>` : ""}</div><iframe title="${english ? "Property location map" : "Mapa de ubicacion de la propiedad"}" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="https://www.google.com/maps?q=${encodeURIComponent(mapQuery)}&output=embed"></iframe></section>`
    : "";
